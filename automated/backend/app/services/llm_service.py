"""
LLM Evaluation Service — Uses Groq (Llama 3.3 70B) for intelligent answer evaluation.
Provides deep semantic understanding instead of simple keyword matching.
"""
import json
import logging
...
try:
    ...
    if settings.GROQ_API_KEY:
        ...
        GROQ_AVAILABLE = True
        logger.info("Groq LLM initialized successfully (Llama 3.3 70B)")
    else:
        logger.warning("GROQ_API_KEY not set. LLM evaluation disabled.")
except ImportError:
    logger.warning("groq package not installed. LLM evaluation disabled.")
except Exception as e:
    logger.warning(f"Groq initialization failed: {e}")


class LLMEvaluationService:
    """Evaluates student answers using Groq's Llama 3.3 70B model."""
    ...
    def _evaluate_chunk(
        ...
        try:
            # Aggregate queries to find general context for the entire chunk if possible
            # Or retrieve context for each question individually
...
[
    ...
    def _parse_batch_response(self, response_text: str, original_questions: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if start != -1 and end != -1:
                ...
                # Try finding { if [ is missing (maybe LLM didn't return an array)
    ...
    def evaluate_unstructured_document(self, exam_id: int, full_text: str, question_paper_text: Optional[str] = None) -> List[Dict]:
        ...
        context = rag_service.query_context(exam_id, full_text[:1000], k=5)
