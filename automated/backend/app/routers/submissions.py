...
from app.services.llm_service import llm_evaluation_service, GROQ_AVAILABLE
...
def process_submission_sync(submission_id: int):
    ...
    try:
        ...
        if not all_question_nos:
            ...
            if answer_keys and GROQ_AVAILABLE:
        ...
        if GROQ_AVAILABLE:
            ...
            logger.warning("GROQ_AVAILABLE is False. Check GROQ_API_KEY in .env")
...
def process_direct_submission_sync(submission_id: int):
    ...
    try:
        ...
        if not results_data:
            logger.error("Direct LLM evaluation failed - no results")
            submission.status = SubmissionStatus.FAILED.value
            db.commit()
            return
